...
*/
            ...
            controller.result = await minify(controller.result);
            controller.result = controller.result.code;
        }
    },

    routeInternalAPIrequestCheck(req) {
    ...
    async routeInternalAPIrequestWork(req, _Controller) {
        ...
        let result = {
            status: "success",
            result: await _Controller['server_' + _Controller._action](...arg)
        };
    ...
    async injectRouteScriptGenerator(req, res) {
        let path = "Controller" + req.path.split('@sf-internal-api')[1].split('/').join('.');
        let _Controller = Application.System.ObjSelector(Application, path);
